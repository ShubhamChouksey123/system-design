...
- **Status updates flow back through the identical pipeline, reversed** — when asked how `sent`/`received`/`read` are communicated between users, the candidate correctly walked it symmetrically: the recipient's device sends its own ack to *its* gateway, which propagates to the Delivery Worker, which looks up **the original sender's** gateway in the Session Store and pushes the status update there. No separate mechanism — a receipt is routed exactly like a message, just addressed the other way.
...
- **Status updates (`sent`/`received`/`read`) reasoned symmetrically, when asked** — the same registry-lookup-and-route logic that delivers a message also carries a receipt back to the *sender*; the candidate stated this directly, and it matches how real chat systems (e.g. WhatsApp's tick-based receipts) are actually built — a receipt is just another message, addressed the other way.
...
| Receipts | **`receipts` Kafka topic** — same Delivery Worker + Session Store routing as messages, reversed direction, kept **separate** from `messages` | **Kafka** |
...
**Problem C — getting `delivered`/`read` status back to the sender.** The candidate answered this correctly when asked: a receipt is **not a new mechanism**, it's the *same* send pipeline addressed the other way.

```text
# 12a. Recipient's client acks over its own WebSocket: {message_id, status: delivered|read}
# 12b. Recipient's gateway emits a receipt event -- to a SEPARATE `receipts` topic, not `messages`
# 12c. The SAME Delivery Worker consumes it (same code path, different topic)
# 12d. gw = session_store.get(ORIGINAL_SENDER_id)     # the lookup direction flips
# 12e. if gw:  publish(topic=gw, receipt)             # online -> sender's owning gateway
#      else:   receipt just updates the cursor; sender sees it on next reconnect/sync
# 12f-g. Sender's gateway pushes the tick update down the sender's socket
```

This is exactly how production chat systems describe their own receipt flow — "read receipts are just tiny system messages flowing the same path in reverse" — so the routing logic (session-store lookup → per-gateway pub-sub → push) is **not duplicated**, only re-entered with sender and recipient swapped. The one real design decision on top is **putting receipts on their own Kafka topic** rather than piggy-backing on `messages`: a group chat generates one receipt *per member* per message (often outnumbering the messages themselves), and losing a stray receipt is harmless in a way that losing a message never is — two different volume and durability profiles that argue for separating the streams, even though the routing code is shared between them.
...
| Getting `delivered`/`read` status back to the sender | **Same session-store lookup + pub-sub routing, reversed** — a receipt is just a message addressed back to the sender, carried on its own `receipts` topic |
...
| **Sync cursor** (Redis/KV) | `cursor:{user_id}:{conversation_id}` | `last_delivered_seq`, `last_read_seq` | the recipient's **own** read/delivered progress — advanced by their ack; separate from the receipt *event* that notifies the sender (§4, Problem C) |
...
| **Receipts on a dedicated `receipts` Kafka topic, reusing the Delivery Worker's routing logic** | Piggyback receipts on the `messages` topic | A group message can generate *many* receipts (one per member) for every one message, and a dropped receipt is harmless — different volume and loss-tolerance than message content, so the streams separate even though the routing code doesn't |
...
8. **A status update is a message addressed the other way — don't invent a second mechanism for it.** The candidate got this right when asked: `delivered`/`read` acks reuse the exact same session-store lookup and per-gateway routing as a normal send, just with sender and recipient swapped. The one refinement worth adding *unprompted* next time: put receipts on their **own** topic, separate from `messages` — group chats generate far more receipts than messages, and losing one is harmless in a way losing a message never is.
