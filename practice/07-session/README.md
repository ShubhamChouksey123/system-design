...
> **What "atomic" actually rests on — and how to defend it.** Don't claim "Redis is single-threaded"; state the precise contract: **a single command, a Lua script (`EVAL`), or a `MULTI/EXEC` transaction executes atomically on the node that owns the keys.** In standard Redis that atomicity comes from **single-threaded command execution** — commands run one-at-a-time on the main thread, so the script's read → decrement → write can't be interrupted. Two follow-ups worth pre-empting: **(1)** *"Didn't Redis 6 add threads?"* — yes, **I/O threads** for the network layer (reading/writing sockets); command *execution* is still serialized, so atomicity holds. **(2)** the guarantee is **per node** — a script may only touch keys in one hash slot, which is exactly why sharding by `client_id` works: one client's bucket lives on one node and that node's thread serializes all its requests. (Multi-threaded forks like KeyDB / Valkey preserve the same atomicity via internal locking — so rely on the *atomicity contract*, not on single-threading per se.)
...
### 8. Going global — multi-region rate limiting

Everything above (§1–7) describes **one region** — say `us-east-2`: a stateless edge tier and one Redis cluster, sharded by `client_id`, all inside that region. The moment the backend deploys to multiple AWS regions and clients are global, a **single** Redis — no matter how it's sharded by key — becomes a **cross-region dependency**: a `client_id` hash always lands on the *same* shard, so a request landing in `eu-central-1` for a client whose shard happens to sit in the `us-east-2` Redis pays a transatlantic round trip just to check a counter. Sharding by key solves *contention*, not *locality* — that's the gap in the session's own single-Redis design.

**The fix: shard by region, not (only) by key.** Deploy the **entire §3 architecture independently in every region** — each region gets its own edge tier and its own **local-only** Redis cluster, holding counters only for the traffic that region itself sees. A request is checked against whichever region's Redis it happens to land in (via GeoDNS or anycast routing to the nearest region) — **zero cross-region calls on the hot path, ever.**

![Multi-region rate limiter topology as a Mermaid flowchart. A global client is routed by GeoDNS or anycast to its nearest region. In the us-east-2 region an API gateway with rate-limit middleware does an atomic check-and-increment against a local-only Redis cluster and forwards allowed requests to that region's backend service. The eu-central-1 region has an identical, fully independent stack: its own gateway, its own local-only Redis cluster, and its own backend service, with no request-path connection between the two regions. An optional slow async rebalancer runs every few seconds, reading each region's reported usage and adjusting each region's local limit share, but never sits on the per-request path.](./diagrams/multi-region.png)

**The trade-off this creates — and why it's the right one here.** If the global limit is meant to be, say, 100 req/s per client, and the design is deployed identically across `N` regions each independently enforcing 100 req/s locally, the **effective worst-case global limit becomes `N × 100 req/s`** — a client that deliberately spreads requests evenly across all `N` regions can get up to `N` times the intended limit before any single region's counter trips. This is exactly the trade this session already made explicitly for the single-region design (§2: **near-accurate, availability over strict consistency**) — it just now applies at the region level instead of the request level. Two ways to handle it, both legitimate:

- **Accept the multiplier (Cloudflare's actual choice).** For abuse protection and fair-usage limiting — this problem's actual framing — a client getting up to `N×` before being throttled is rarely worth paying cross-region latency on every request to prevent, especially because GeoDNS/anycast naturally keeps most clients' traffic concentrated in one nearby region anyway. Enforce the *full* limit independently, per region.
- **Divide the limit by `N` up front.** If the global cap must hold even against a client that deliberately spreads across every region, configure each region's *local* limit as `global_limit ÷ N`. Simple and exact in the worst case, but overly strict for the common case (a client who only ever calls one region is now capped at `1/N` of the limit they should be entitled to).

**Tightening it further, without ever touching the hot path.** If a static split is too coarse (regional traffic share shifts over time — most of a client's calls come from `us-east-2` today, `eu-central-1` next month), add a **slow, async control loop**: each region periodically (every few seconds) reports its observed usage to a lightweight coordinator, which redistributes each region's *local limit share* based on real traffic patterns — shown as the dashed edges in the diagram above. This is the same "hierarchical allocation with a slow rebalancing loop" pattern that large-scale multi-region systems use generally: the *correction* happens out-of-band on a control plane, so the *enforcement* decision for every single request still only ever reads its own region's local Redis.

**When the multiplier genuinely isn't acceptable** — a hard, paid-quota billing cap rather than abuse protection — that's the one case worth reconsidering: route each `client_id` to one consistent "home region" (so the common case still never crosses regions) and only forward the rare cross-region request to its home region's Redis for the authoritative check, rather than giving up locality for every request to get correctness for a few. This mirrors the earlier CP-vs-AP call (§7, "near-accurate vs. strictly consistent") — pay for cross-region consistency only on the specific path where overshoot is actually unacceptable, never as the default.
...
7. **Sharding a counter by key solves contention, not locality.** A single Redis sharded by `client_id` still forces a cross-region round trip for whichever region doesn't own that shard. Going multi-region means deploying the *whole* stack per region with a **local-only** Redis, which turns the global limit into `N_regions × local_limit` in the worst case — state that multiplier explicitly, and divide the limit by `N` up front only if a hard cap (not abuse protection) truly can't tolerate it.
